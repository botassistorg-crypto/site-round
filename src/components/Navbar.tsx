    ...
    >
      <div className="container mx-auto px-6 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-2 group relative z-50">
          <div className="bg-indigo-600 p-2 rounded-xl shadow-lg shadow-indigo-500/20 group-hover:bg-indigo-700 transition-colors">
            <Rocket className="w-5 h-5 text-white" />
          </div>
          <span className={`text-xl font-bold tracking-tight ${scrolled || isOpen ? "text-slate-900" : "text-slate-900 md:text-white"}`}>
            SiteRound
          </span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-1">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.path}
              className={`px-4 py-2 text-sm font-medium rounded-full transition-all hover:bg-indigo-50/10 hover:backdrop-blur-sm ${
                scrolled ? "text-slate-600 hover:text-indigo-600" : "text-slate-300 hover:text-white"
              }`}
            >
              {link.name}
            </Link>
